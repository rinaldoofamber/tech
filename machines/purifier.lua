...
local function inactive_formspec(energy_percent)
  local result = "size[8,9]"..
    ...
    "list[current_name;cell;3.75,3.5;1,1;]" ..
...
local function on_timer(pos)
  ...
  if inv then
    ...
    local res = tech.get_craft({type = "purify", recipe = item:get_name()})
    local spends_cells = res.cells
    if res and inv:room_for_item("dst", ItemStack(res.output)) and inv:contains_item("cell", ItemStack("tech:water_cell " .. spends_cells)) then
      if selfpow:get_energy() > spends then
        tech.play_sound(pos, "tech_purifier", 6)
        ...
        if proc:get_progress() + 1 < proc:get_total_progress() then
          ...
          local cellstack = inv:get_stack("cell", 1)
          cellstack:take_item(spends_cells)
          inv:set_stack("cell", 1, cellstack)
...
local function can_dig(pos, player)
	local meta = minetest.get_meta(pos)
	local inv = meta:get_inventory()
	return inv:is_empty("dst") and inv:is_empty("src") and inv:is_empty("cell")
end

minetest.register_node("tech:purifier", {
...
tech.register_consumer("tech:purifier")

minetest.register_abm({
  nodenames = {"tech:purifier"},
  interval = 1,
  chance = 1,
  action = on_timer,
})
...
minetest.register_craft({
  output = 'tech:purifier 1', 
  recipe = {
  {'', 'tech:sqeezer', ''},
  {'', 'tech:advanced_machine_casing', ''},
  {'tech:microtubes', 'tech:auto_grinder', 'tech:microtubes'},
  }
})
