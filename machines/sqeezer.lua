...
local function on_timer(pos)
  ...
  if inv then
    ...
    local res = tech.get_craft({type = "sqeeze", recipe = item:get_name()})
    if res and inv:room_for_item("dst", ItemStack(res.output)) and inv:contains_item("cell", ItemStack("tech:cell")) then
      if selfpow:get_energy() > spends then
        ...
        if proc:get_progress() + 1 < proc:get_total_progress() then
          ...
          inv:remove_item("cell", ItemStack("tech:cell"))
...
minetest.register_node("tech:sqeezer", {
  description = "Sqeezer",
...
tech.register_consumer("tech:sqeezer")

minetest.register_abm({
  nodenames = {"tech:sqeezer"},
  interval = 1,
  chance = 1,
  action = on_timer,
})
